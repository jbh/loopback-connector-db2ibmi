...
db.execute_prepared = function (stmt, sql, bind, cb) {
  stmt.prepare(sql, function(err){
    stmt.bindParam(bind, function(err){
      if(err) return cb(err)
      stmt.execute(function(x1, x2, x3, x4) {
        stmt.fetchAll(function(res, err){
          cb(err,res)
        });
      });
    });
  ...
  function executeStatement(conn, cb) {
    ...
    if (params.length > 0) {
      ...
      params.forEach(function(param) {
        bind.push([param, db.SQL_PARAM_INPUT, count++]);
      });
      
      db.execute_prepared(stmt, sql, bind, function(err, res) {
        cb && cb(err, res);
      });
    } else {
      db.query(stmt, sql, function(err, res) {
        cb && cb(err, res);
      });
    }
  }
...
DB2IBMi.prototype.parseDate = function(date) {
  ...
  var prefix = offset >= 0 ? "+" : "-";
  offset = (prefix) +
  ("00" + parseInt(offset / 60)).substr(-2,2) +
  ":" + ("00" + offset % 60).substr(-2,2);
